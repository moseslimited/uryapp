...
def print_pos_page(doctype, name, print_format):
    if not name:
        frappe.throw(_("Invoice name is required for print."))
    ...
    restaurant_table, branch, inv_name = values
    print_channel = "{}_{}".format("print", branch or "default")
    frappe.publish_realtime(print_channel, {"data": data})

    invoice_printed = frappe.db.get_value("POS Invoice", inv_name, "invoice_printed")
    if invoice_printed == 0:
        frappe.db.set_value("POS Invoice", inv_name, "invoice_printed", 1)
