...
def _expand_sync_items_for_kot(items):
    """Flatten cart lines so KOT diff sees main + included modifier lines (matches saved invoice rows)."""
    ...
    for d in items:
        if not isinstance(d, dict):
            continue
        out.append(d)
        parent_qty = flt(d.get("qty"))
        for m in d.get("included_modifiers") or []:
            icode = (m.get("item") or "").strip()
            if not icode:
                continue
            qty = parent_qty * flt(m.get("qty_factor") or 1)
            if qty <= 0:
                continue
            glabel = (m.get("group_label") or m.get("group_id") or "").strip()
            out.append(
                {
                    "item": icode,
                    "item_name": m.get("item_name") or icode,
                    "qty": qty,
                    "comment": _("Included: {0}").format(glabel) if glabel else _("Included"),
                }
            )
    return out


def _auto_consolidate_pos_invoice(invoice_name: str):
    """Create consolidated Sales Invoice/Credit Note immediately for one submitted POS Invoice.

    Idempotent:
    - if POS Invoice is already linked, returns existing consolidated invoice.
    """
    if not invoice_name:
        frappe.throw(_("POS Invoice is required for consolidation."))

    pos_inv = frappe.get_doc("POS Invoice", invoice_name)
    if pos_inv.docstatus != 1:
        frappe.throw(_("Only submitted POS Invoices can be consolidated."))

    if getattr(pos_inv, "consolidated_invoice", None):
        return pos_inv.consolidated_invoice

    merge_log = frappe.new_doc("POS Invoice Merge Log")
    merge_log.posting_date = pos_inv.posting_date
    merge_log.posting_time = pos_inv.posting_time
    merge_log.company = pos_inv.company
    merge_log.customer = pos_inv.customer
    merge_log.append(
        "pos_invoices",
        {
            "pos_invoice": pos_inv.name,
            "is_return": cint(getattr(pos_inv, "is_return", 0)),
            "return_against": getattr(pos_inv, "return_against", None),
        },
    )
    ...
    pos_inv.reload()
    if not getattr(pos_inv, "consolidated_invoice", None):
        frappe.throw(_("Failed to create linked Sales Invoice for POS Invoice {0}.").format(pos_inv.name))
...
def get_order_invoice(table=None, invoiceNo=None, order_type=None, is_payment=None):
    ...
    # Settling payment from Orders (or any screen): load draft by invoice name.
    # Do not require restaurant_table to match — UI may omit table while the doc has one set.
    if is_payment == "Payments" and invoiceNo and frappe.db.exists("POS Invoice", invoiceNo):
        inv = frappe.get_doc("POS Invoice", invoiceNo)
        if inv.docstatus == 0:
            return inv
        frappe.throw(
            _("This invoice is already submitted or cancelled and cannot be paid again."),
            title=_("Cannot pay"),
        )
    ...
    if table:
        if is_payment == "Payments":
            ...
            if invoiceNo:
                ...
                # New order at table (multiple orders per table allowed)
                invoice_name = None
        ...
        # Use Standard Selling for all non-Aggregator orders (no Drinks/Food-specific lists)
        invoice.selling_price_list = "Standard Selling"
...
def sync_order(
    ...
    if (
        last_invoice
        and frappe.db.get_value(inv_dt, last_invoice, "invoice_printed") == 1
    ...
    if last_invoice and last_modified_time:
        ...
        # Only block when updating a draft order for a table (not Take Away) and user is not billing
        if table and invoice.name and invoice.invoice_printed == 0 and not billing_user:
            frappe.msgprint(
                title="Table occupied",
                indicator="red",
                msg=("{0} is already occupied. Please refresh the page.").format(table),
            )
            return {"status": "Failure"}
    ...
    if staff_code:
        invoice.custom_staff_code = staff_code
    if staff_name:
        invoice.custom_staff_name = staff_name
    if staff_user:
        invoice.custom_staff_user = staff_user
        if not waiter:
            invoice.waiter = staff_user
    ...
    active_menu = frappe.db.get_value("URY Restaurant", restaurant, "active_menu") if restaurant else None
    fallback_price_list = frappe.db.get_value(
        "Price List", dict(restaurant_menu=active_menu, enabled=1)
    ) if active_menu else None
    ...
    for d in items:
        if not isinstance(d, dict):
            continue
        main_item = (d.get("item") or "").strip()
        if not main_item:
            continue

        course = frappe.db.get_value(
            "URY Menu Item", {"item": main_item, "parent": menu}, "course"
        )
        is_giveaway = cint(d.get("is_giveaway"))

        # Use rate from frontend (e.g. edited price in POS) when provided; giveaway is always 0
        rate_from_request = d.get("rate") is not None and d.get("rate") != ""
        if rate_from_request:
            try:
                rate_value = float(d.get("rate"))
        ...
        if is_giveaway:
            ...
            line_rate = rate_value
        else:
            item_prices = frappe.db.get_list(
                "Item Price",
                filters={"item_code": main_item, "price_list": price_list},
                fields=["price_list_rate"],
            )
            if not item_prices and fallback_price_list and fallback_price_list != price_list:
                item_prices = frappe.db.get_list(
                    "Item Price",
                    filters={"item_code": main_item, "price_list": fallback_price_list},
                    fields=["price_list_rate"],
                )
            if not item_prices:
                frappe.throw(
                    ...
                    ).format(main_item, price_list)
                )
            line_rate = item_prices[0].price_list_rate

        line_kind = "Giveaway" if is_giveaway else "Normal"
        ...
        invoice.append(
            ...
            dict(
                ...
                **({"custom_course": course} if course else {}),
                ...
                base_price_list_rate=line_rate,
                cost_center=cost_center,
                custom_ury_line_kind=line_kind,
            ),
        )
        ...
        for m in d.get("included_modifiers") or []:
            ...
            if not icode:
                continue
            if not frappe.db.exists("Item", icode):
                frappe.throw(_("Included modifier item {0} not found.").format(icode))
            factor = flt(m.get("qty_factor") or 1)
            mq = parent_qty * factor
            if mq <= 0:
                continue
            glabel = (m.get("group_label") or m.get("group_id") or "").strip()
            mod_comment = _("Included: {0}").format(glabel) if glabel else _("Included")
            iname = m.get("item_name") or frappe.db.get_value("Item", icode, "item_name")
            mod_course = frappe.db.get_value(
                "URY Menu Item", {"item": icode, "parent": menu}, "course"
            )
            invoice.append(
                "items",
                dict(
                    ...
                    item_name=iname,
                    qty=mq,
                    **({"custom_course": mod_course} if mod_course else {}),
                    comment=mod_comment,
                    rate=0,
                    price_list_rate=0,
                    base_price_list_rate=0,
                    cost_center=cost_center,
                    custom_ury_line_kind="Included",
                ),
            )
    ...
    try:
        kot_execute(
            ...
            customer,
            table,
            _expand_sync_items_for_kot(items),
            past_item,
            comments,
        )
    ...
    # table status: mark table as occupied when it has an unpaid (draft) order
...
# Method for URY POS
@frappe.whitelist()
def make_invoice(customer, payments, cashier, pos_profile, owner, additionalDiscount=None, table=None, invoice=None, staff_code=None, staff_user=None, staff_name=None, tip_amount=None, service_charge_amount=None, waiter=None):
    ...
    if staff_name:
        invoice.custom_staff_name = staff_name
    # Waiter: from payment dialog selection, or fallback to staff_user
    if waiter:
        invoice.waiter = waiter
    elif staff_user:
        invoice.waiter = staff_user
    ...
    # Apply tip and service charge (custom fields) and add to totals
    tip = flt(tip_amount, 2)
    service_charge = flt(service_charge_amount, 2)
    if tip > 0 or service_charge > 0:
        if hasattr(invoice, "custom_tip_amount"):
            invoice.custom_tip_amount = tip
        if hasattr(invoice, "custom_service_charge_amount"):
            invoice.custom_service_charge_amount = service_charge
        invoice.grand_total = flt(invoice.grand_total, 2) + tip + service_charge
        invoice.rounded_total = round(invoice.grand_total, 0)
        invoice.outstanding_amount = invoice.rounded_total
    ...
    # Do not set invoice.owner on existing doc - Frappe does not allow changing "Created By"
    invoice.save()
    try:
        invoice.submit()
        _auto_consolidate_pos_invoice(invoice.name)
    except Exception as e:
