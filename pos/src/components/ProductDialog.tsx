...
import { OrderItem, usePOSStore, type IncludedModifierSelection } from '../store/pos-store';
...
const ProductDialog: React.FC<ProductDialogProps> = ({
  ...
  const effectiveInitialPrice = initialPrice ?? (itemToReplace?.selectedVariant?.price ?? itemToReplace?.price);
  const [customPrice, setCustomPrice] = useState<string>(effectiveInitialPrice != null ? String(effectiveInitialPrice) : '');
  ...
  const [selectedIncluded, setSelectedIncluded] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    ...
    const pickMods = (source: OrderItem | undefined) => {
      if (!source?.includedModifiers?.length || !groups.length) return null;
      const init: Record<string, string> = {};
      for (const g of groups) {
        const found = source.includedModifiers!.find(
          (im) => im.group_id === g.group_id || im.group_label === g.label
        );
        if (found) init[g.group_id] = found.item;
      }
      return init;
    };
    if (editMode && itemToReplace) {
      setSelectedIncluded(pickMods(itemToReplace) ?? {});
    } else if (!editMode && existingCartItem) {
      setSelectedIncluded(pickMods(existingCartItem) ?? {});
    } else {
      setSelectedIncluded({});
    }
  }, [selectedItem, editMode, itemToReplace, existingCartItem]);
  ...
  // Initialize quantity, comments, and price from cart when in edit mode
  useEffect(() => {
    if (editMode && itemToReplace) {
      const p = itemToReplace.selectedVariant?.price ?? itemToReplace.price;
      setCustomPrice(p != null ? String(p) : '');
    }
  }, [editMode, itemToReplace]);
  ...
  // In edit mode allow custom price; otherwise use menu price
  const basePrice = editMode && customPrice !== ''
    ? (parseFloat(customPrice) || 0)
    : (selectedItem?.price ? Number(selectedItem.price) : 0);
  ...
  const handleAddToOrder = () => {
    ...
    for (const g of modifierGroups) {
      const min = g.min ?? (g.items?.length ? 1 : 0);
      if (min > 0 && !selectedIncluded[g.group_id]) {
        setItemError(`Please select: ${g.label || g.group_id}`);
        return;
      }
    }
    setItemError(null);
    ...
    const includedModifiers: IncludedModifierSelection[] = modifierGroups
      .map((g) => {
        const code = selectedIncluded[g.group_id];
        if (!code) return null;
        const mi = menuItems.find((x) => x.item === code);
        return {
          item: code,
          item_name: mi?.item_name || mi?.name || code,
          qty_factor: 1,
          group_id: g.group_id,
          group_label: g.label,
        };
      })
      .filter((x): x is IncludedModifierSelection => x !== null);
    ...
    const orderItem: OrderItem = {
      ...
      includedModifiers: includedModifiers.length ? includedModifiers : undefined,
    };
    ...
    <Dialog open={true} onOpenChange={handleClose}>
      ...
      >
        ...
        <div className="md:w-1/3 p-6 overflow-y-auto">
          ...
          {editMode && (
            <div className="mt-6">
              <h3 className="text-lg font-semibold mb-3">Price (per unit)</h3>
              <Input
                type="number"
                min="0"
                step="any"
          ...
          {modifierGroups.length > 0 && (
            <div className="mt-6 space-y-4">
              <h3 className="text-lg font-semibold">Included choices</h3>
              {modifierGroups.map((g) => (
                <div key={g.group_id}>
                  <p className="text-sm font-medium text-gray-700 mb-2">
                    {g.label}
                    {(g.min ?? (g.items?.length ? 1 : 0)) > 0 && (
                      <span className="text-red-500"> *</span>
                    )}
                  </p>
                  <div className="flex flex-wrap gap-2">
                    {(g.items || []).map((code) => {
                      const mi = menuItems.find((m) => m.item === code);
                      const label = mi?.item_name || mi?.name || code;
                      const selected = selectedIncluded[g.group_id] === code;
                      return (
                        <button
                          key={code}
                          type="button"
                          onClick={() =>
                            setSelectedIncluded((prev) => ({ ...prev, [g.group_id]: code }))
                          }
                          className={cn(
