...
const AuthGuard: React.FC<Props> = ({ children }) => {
  ...
  if (!hasAccess) {
    const requiredRoles = posProfile.role_allowed_for_billing?.map(r => r.role).filter(Boolean) || [];
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center max-w-md">
          ...
          {requiredRoles.length > 0 ? (
            ...
            <p className="text-sm text-gray-500 mt-2">No roles are set for this POS Profile. In Desk, open your POS Profile and add at least one role under &quot;Role Allowed For Billing&quot; (e.g. Sales User), then assign that role to your user.</p>
          )}
          <Button 
            variant="outline"
            className="mt-4"
