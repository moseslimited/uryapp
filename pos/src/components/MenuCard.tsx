...
interface MenuCardProps {
  ...
  course?: string;
  item: string;
  /** Stock quantity in POS warehouse; green if >= 10, red if < 10 */
...
const MenuCard: FC<MenuCardProps> = ({ 
  ...
  const qtyColor = actual_qty != null
    ? actual_qty > 10
      ? 'text-green-600'
      : 'text-red-600'
    : '';
  return (
    ...
    >
      {/* Image section - image fills box (cover) to avoid gray background */}
      ...
      <div className="flex-1 p-3 flex flex-col">
        ...
        {/* Quantity badge (POS warehouse): green if > 10, red if <= 10 */}
        {actual_qty != null && (
          <div className="mt-1">
            <span className={cn("text-xs font-medium tabular-nums", qtyColor)}>
              Qty: {Math.round(actual_qty)}
            </span>
          </div>
        )}
