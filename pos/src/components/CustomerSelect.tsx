  ...
  async function handleAddCustomerSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const customerData: CreateCustomerData = {
        ...
        custom_is_restaurant_customer: true, // So they appear in POS guest list and we can sell to them
      };
...
export function CustomerSelect({ disabled }: CustomerSelectProps) {
  ...
  // Debounced search - also fetch initial customers when dropdown opens
  useEffect(() => {
    if (!isOpen) {
      setSearchResults([]);
      setSearchError(null);
    ...
    }, searchTerm.trim() ? 300 : 0); // No delay when showing initial customers
        ...
        <div className="relative">
          <div className="flex items-center relative">
            <input
              ...
              onFocus={() => {
                setIsOpen(true);
                // Trigger search immediately when focused with empty search
            ...
            <div className="absolute w-full mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto">
              {isSearching && (
                <div className="flex items-center justify-center p-4 text-gray-500 text-sm select-none">
                  <Loader className="w-4 h-4 mr-2 animate-spin" /> {searchTerm.trim() ? 'Searching...' : 'Loading customers...'}
                </div>
              )}
              {searchError && (
