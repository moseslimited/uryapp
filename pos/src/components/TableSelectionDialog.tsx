...
import { getRooms, getTables, freeTable, Room, Table } from '../lib/table-api';
...
const getTableStatus = (table: Table): { label: string; badgeClass: string } => {
  if (table.occupied !== 1) return { label: 'Free', badgeClass: 'bg-green-100 text-green-800' };
  if (table.customers_served === 1) return { label: 'Served', badgeClass: 'bg-blue-100 text-blue-800' };
  return { label: 'Occupied', badgeClass: 'bg-amber-100 text-amber-700' };
};
...
const TableSelectionDialog: React.FC<Props> = ({ onClose }) => {
  ...
  const [freeingTableName, setFreeingTableName] = useState<string | null>(null);
  ...
  const refreshTablesForRoom = async (room: string) => {
    const fetchedTables = await getTables(room);
    const sortedTables = sortTables(fetchedTables);
    setTables(sortedTables);
    setTablesCache(prev => ({ ...prev, [room]: fetchedTables }));
  };

  const handleFreeTable = async (e: React.MouseEvent, tableName: string) => {
    e.preventDefault();
    e.stopPropagation();
    if (!selectedRoom) return;
    setFreeingTableName(tableName);
    try {
      await freeTable(tableName);
      showToast.success('Table freed');
      await refreshTablesForRoom(selectedRoom);
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Could not free table';
      showToast.error(message);
    } finally {
      setFreeingTableName(null);
    }
  };
            ...
            <div className="grid grid-cols-3 gap-6">
              {tables.map(table => {
                ...
                >
                  ...
                  className={cn(
                    'w-8 h-8',
                    table.occupied === 1 ? (table.customers_served === 1 ? 'text-blue-500' : 'text-amber-500') : 'text-gray-500'
                  )} />
                  <div className="text-center">
                    <div className="font-medium">{table.name}</div>
                    <div className="mt-2 h-4">
                      {table.occupied === 1 && (
                        <Badge variant="secondary" className={cn('text-xs', status.badgeClass)}>
                          {status.label}
                        </Badge>
                      )}
                    </div>
                    {table.occupied === 1 && (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="mt-2 text-xs h-7 px-2 border-gray-300 hover:bg-gray-100"
                        onClick={(e) => handleFreeTable(e, table.name)}
                        disabled={freeingTableName === table.name}
                      >
                        {freeingTableName === table.name ? 'Freeing…' : 'Free'}
                      </Button>
                    )}
                  </div>
                </Button>
