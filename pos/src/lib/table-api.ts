...
/** Mark the table's current order as served (food delivered). */
export async function setCustomersServed(tableName: string): Promise<void> {
  const { call } = await import('./frappe-sdk');
  await call.post('ury.ury_pos.api.set_customers_served', { table_name: tableName });
}

/** Free the table (set occupied=0) after customers have left. Use after payment and/or mark as served. */
export async function freeTable(tableName: string): Promise<void> {
  const { call } = await import('./frappe-sdk');
  await call.post('ury.ury_pos.api.free_table', { table_name: tableName });
} 