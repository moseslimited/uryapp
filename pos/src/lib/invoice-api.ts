...
export interface POSInvoice {
  ...
  customer: string;
  status: 'Draft' | 'Recently Paid' | 'Paid' | 'Pay Later' | 'Consolidated' | 'Return';
...
export async function createPosReturn(invoiceName: string): Promise<{ name: string; message: string }> {
  return call.post('ury.ury_pos.api.create_pos_return', { invoice_name: invoiceName }) as Promise<{ name: string; message: string }>;
} 