...
> = (set, get) => ({
  ...
  checkAccess: () => {
    ...
    // If no roles are configured on the POS Profile, allow any logged-in user (no restriction)
    if (!allowedRoles.length) {
      set({ hasAccess: true });
      return;
    }

    // Otherwise require the user to have at least one of the allowed roles
