...
export interface OrdersState {
  ...
  selectedStatus: 'Draft' | 'Recently Paid' | 'Paid' | 'Pay Later' | 'Consolidated' | 'Return';
...
> = (set, get) => ({
  ...
  fetchOrders: async (page = 1) => {
    try {
      ...
      if (currentSelected) {
        const refreshedSelected = (invoices || []).find((inv) => inv.name === currentSelected.name);
        if (!refreshedSelected) {
          get().clearSelectedOrder();
        } else {
          set({ selectedOrder: refreshedSelected });
        }
      }
    } catch (error) {
      set({ 
